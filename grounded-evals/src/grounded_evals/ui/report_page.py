"""Outputs page for the judge spec and LLM-as-Judge gate."""
...
<div class="footer">Generated by GEDD (Grounded Evidence Driven Development)</div>
</body>
</html>"""
...
def report_page():
        ...
        with ui.element("main").classes("dynamic-page"):
            with ui.element("div").classes("empty-state-panel"):
                ...
                ui.html(
                    ...
                    "SME_error_analysis.md, the judge spec, and the gate as one evidence-backed output set."
                    "</div>"
                )
    ...
    def download_requirements_md():
        if not requirements_markdown.strip():
            ui.notify("Annotate failures before generating the judge spec", type="warning")
            return
        ui.download(requirements_markdown.encode(), "judge-spec.md")
    ...
    with ui.element("main").classes("dynamic-page"):
        with ui.element("section").classes("dynamic-hero"):
            with ui.element("div"):
                ...
                ui.html(
                    ...
                    "the same SME evidence for the judge spec and LLM-as-Judge gate."
                    "</div>"
                )
        ...
        with ui.element("section").classes("metric-strip"):
            for value, label in [
                ...
                ("ready" if requirements_markdown else "missing", "Judge spec"),
            ...
            with ui.element("div").classes("rr-action-card"):
                with ui.row().classes("items-center justify-between gap-3 flex-wrap"):
                    ...
                    with ui.row().classes("gap-2 flex-wrap"):
                        ...
                        ui.button(
                            "Download judge spec", icon="description", on_click=download_requirements_md
        ...
        with ui.element("div").classes("page-card"):
            ...
            ui.label(
                "Export SME_error_analysis.md as the shared evidence handoff, then use it for the judge spec and LLM-as-Judge gate."
