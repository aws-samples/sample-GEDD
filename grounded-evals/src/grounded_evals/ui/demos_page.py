...
def _build_domain_registry():
    ...
    # ── AAA game localization release gate ────────────────────────────────
    try:
        ...
        domains[:0] = [
            {
                "id": "aaa_game_localization",
                "name": "AAA Game Localization Agent",
                "icon": "translate",
                "operator": "AAA Game Localization",
                "tagline": "AAA Game localization assistant - lore glossary, runtime tokens, RTL controls, store copy, ratings gates",
                ...
                "loader": load_aaa_game_localization_demo,
                "codebook": AAA_GAME_LOCALIZATION_CODEBOOK,
                "sample_queries": [
                    {
                        "q": "German strings use inconsistent terms for rift gate. Can we ship?",
                        "verdict": "incorrect",
                        "note": "Agent treated approved franchise lore terminology as optional style.",
                    },
                    {
                        "q": "Japanese subtitle removed {HeroName} and a diplomatic choice tag for timing.",
                        "verdict": "incorrect",
                        "note": "Agent approved runtime token and choice-state markup loss.",
                    },
                    {
                        "q": "Arabic RTL party ability wheel maps bumper prompts to the wrong ally.",
                        "verdict": "incorrect",
                        "note": "Agent dismissed a gameplay-blocking controller mapping defect.",
                    },
                    {
                        "q": "Store copy says the legacy bundle includes all future releases.",
                        "verdict": "incorrect",
                        "note": "Agent allowed localized product-scope and entitlement drift.",
                    },
                ],
                "paradigm_phenomenon": "Fluent AAA Game Localization That Breaks Lore, Runtime, Or Release Compliance",
                "paradigm_consequence": "Lore inconsistency, broken choice-state strings, wrong party controls, refund risk, ratings exposure",
                "judge_prompt": AAA_GAME_LOCALIZATION_JUDGE_PROMPT,
                "pass_rates": [int(r["pass_rate"].rstrip("%")) for r in AAA_GAME_LOCALIZATION_EVAL_HISTORY],
                "n_queries": 8,
                "n_codes": 7,
            }
        ]
