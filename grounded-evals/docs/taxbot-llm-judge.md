...
The TaxBot scenario can be loaded in GEDD with all artifacts described here — 12 golden queries, 8 annotations, 5 codebook entries, 6 coding annotations, the paradigm model, and the generated judge prompt.
...
Load TaxBot from the home page → walk through Eval → Tag → Root Causes → Build Judge → Export. The exported prompt matches Step 4 above.
