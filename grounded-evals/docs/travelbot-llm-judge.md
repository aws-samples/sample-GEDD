...
The TravelBot demo in GEDD ships with all the artifacts described in this post — golden queries across adversarial, edge-case, happy-path, and multi-turn scenarios; human annotations with error codes; the full 10-entry codebook; coding annotations with severity and memos; the paradigm model for Policy Hallucination; and the generated judge prompt. Open the demo and click through each tab to see the full pipeline.
...
Load TravelBot from the home page → Eval tab → Build Judge → Export. The exported prompt is exactly what Step 4 produces above.
